...
int main()
{
	...
	std::ofstream file{"data/avar.dat"};
	file<<"#a dl0 dl1 dl2 ... \n";
	for(uint64_t m=0;m<M;m++){
		file<<a1+m*ha;
		for (int l=0 ; l<=lmax;l++){
...
int main1()
{
	...
	std::cout << "Probelm constants:"
			  ...
			  << "\nlam_wait_2: " << lam_wait_2
			  << "\nPotential values"
			  << "\nV_LJ(rmax):       " << V_LJ(rmax1)
			  << "\nV_rot(rmax,1):    " << hbar2_2m * V_rot(rmax1, 1)
			  << "\nV_rot(rmax,lmax): " << hbar2_2m * V_rot(rmax1, lmax)
			  << "\n"
			  << std::endl;
	...
	for (uint64_t m = 0; m < M; m++)
	{
		...
		// int l=0;
		{
			...
			for (uint64_t i = 2; i < N && i <= ir2; i++)
			{
				...
				// New k
				k2 = 1. / hbar2_2m *
					 (E - ((x < rmax) ? V_LJ(x) + hbar2_2m * V_rot(x, l) : hbar2_2m * V_rot(x, l)));
	...
	std::ofstream file{"data/rmaxvar.dat"};
	file << "#rmax dl0 dl1 dl2 ... \n";
	for (uint64_t m = 0; m < M; m++)
	{
		file << rmax1 + m * hrmax;
		for (int l = 0; l <= lmax; l++)
