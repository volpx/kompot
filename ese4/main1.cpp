...
// WF
double WF(const Vec3D pos[], const size_t N, const double b)
{
	double sum = 0;
	for (size_t j = 0; j < N; j++)
	{
		...
		{
			sum += -0.5 * std::pow(b / (pos[i] - pos[j]).norm(), 5.0);
			// std::cout<<(pos[i] - pos[j]).norm()<<std::endl;
		}
	}
	return std::exp(sum);
}

double Density(const Vec3D pos[], const size_t N, const double b)
{
	// std::cout<<WF(pos, N, b)<<std::endl;
	double wf = WF(pos, N, b);
	return wf * wf;
}
...
int main()
{
	...
	// Parameter mesh
	constexpr double ba = 0.9;
	constexpr double bb = 1.3;
	constexpr double bh = (bb-ba)/M;
	...
	// Uniform random distributed vector
	double delta = 0.11;
	...
	for (uint32_t m = 0; m < M; m++)
	{
		...
		for (uint32_t s = 0; s < S; s++)
		{
			...
			if (s % K == 0)
			{
				// std::cout << average(probs.data, K) << std::endl;
				delta *= (1 + 1e-2 * (average(probs.data, K) / 0.8 - 1));
			}
		...
		std::cout
			<< "Delta: " << delta
			<< "\nLocal kinetic energy: " << average(Ts.data, Sdata)
			<< "\nJF    kinetic energy: " << average(TJFs.data, Sdata)
			<< "\nPotential     energy: " << average(Vs.data, Sdata)
			<<"\n"
			<< std::endl;
	}
