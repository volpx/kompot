...
int main(int argc, char** argv)
{
	...
	{
		Job j{0,20,30};
		MPI_Send(&j,sizeof(Job),MPI_BYTE,1,MPI_TAG_JOB,MPI_COMM_WORLD);
	}
	return 0;
	...
	const double rhoa = 0.2;
	const double rhob = rho_experimental;
	...
	{	
		...
		if(std::abs(Eavg-EJFavg) < 0.1 || tries > 20)
		{
